...
export default function POS() {
 ...
 className={`px-6 py-3 rounded-[var(--radius-md)] font-semibold transition-all ${
 activeTab === 'direct' 
 ? 'brand-btn-primary shadow-md' 
 : 'bg-[var(--color-brand-surface)] text-[#42493e] border border-[var(--color-brand-border)] hover:bg-[#f3f4f5]'
 }`}
 >
 Direct Sale (Walk-in)
 </button>
 ...
 {/* Products List */}
 <div className="lg:col-span-2 brand-card-level-1 p-6">
 <h2 className="text-[20px] font-bold font-heading text-[#191c1d] mb-4">Available Products</h2>
 <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
 {products.map(product => (
 <div key={product._id} className="border border-[var(--color-brand-border)] rounded-[var(--radius-lg)] p-4 flex flex-col justify-between hover:shadow-[0_4px_12px_rgba(45,90,39,0.05)] transition-shadow">
 <div>
 <h4 className="font-semibold text-[#191c1d] mb-1 leading-snug">{product.name}</h4>
 <p className="text-[var(--color-brand-primary)] font-bold mb-2">₹{product.price}</p>
 <p className="text-xs text-[#72796e] mb-4">Stock: {product.stock}</p>
 </div>
 <button 
 onClick={() => addToCart(product)}
 disabled={product.stock <= 0}
 className="w-full py-2 brand-btn-secondary text-sm disabled:opacity-50"
 >
 Add
 </button>
 </div>
 ...
 <h4 className="text-xl font-bold text-gray-900 mb-2">Sale Completed!</h4>
 <p className="text-gray-500 mb-4">Receipt generated for</p>
 <div className="bg-emerald-50 px-6 py-3 rounded-xl mb-6">
 <span className="text-3xl font-mono font-bold tracking-widest text-emerald-600 ">
 {completedDirectOrderCode}
 </span>
 </div>
 <button 
 ...
 <span className="text-2xl font-bold text-emerald-600 ">₹{directTotalAmount}</span>
 </div>
 ...
 directPaymentMethod === 'UPI' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-gray-200 text-gray-500 hover:border-emerald-300'
 }`}
 >
 ...
 className={`flex flex-col items-center justify-center p-3 border-2 rounded-xl transition-all ${
 directPaymentMethod === 'Cash' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-gray-200 text-gray-500 hover:border-emerald-300'
 }`}
 >
 ...
 <h2 className="text-xl font-bold text-gray-900 mb-4">Fetch Online Order</h2>
 ...
 className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:outline-none text-lg font-mono tracking-wider"
 value={orderCode}
 ...
 <button 
 onClick={searchOrder}
 className="px-8 py-3 bg-gray-900 hover:bg-gray-800 text-white font-semibold rounded-xl transition-colors"
 >
 Fetch
 </button>
 </div>
 ...
 paymentMethod === 'UPI' ? 'border-emerald-500 bg-emerald-50 ' : 'border-gray-200 hover:border-emerald-300'
 }`}
 >
 <QrCode className={`mr-3 ${paymentMethod === 'UPI' ? 'text-emerald-500' : 'text-gray-400'}`} />
 <span className={`font-medium ${paymentMethod === 'UPI' ? 'text-emerald-700 ' : 'text-gray-600 '}`}>UPI Scan</span>
 </button>
 <button 
 onClick={() => setPaymentMethod('Cash')}
 className={`w-full flex items-center p-4 border-2 rounded-xl transition-all ${
 paymentMethod === 'Cash' ? 'border-emerald-500 bg-emerald-50 ' : 'border-gray-200 hover:border-emerald-300'
 }`}
 >
 <Banknote className={`mr-3 ${paymentMethod === 'Cash' ? 'text-emerald-500' : 'text-gray-400'}`} />
 <span className={`font-medium ${paymentMethod === 'Cash' ? 'text-emerald-700 ' : 'text-gray-600 '}`}>Cash</span>
 </button>
 </div>
 ...
 <div className="flex flex-col items-center justify-center py-8 text-center">
 <CheckCircle2 size={64} className="text-green-500 mb-4" />
 <h4 className="text-xl font-bold text-gray-900 mb-2">Paid via {order.paymentMethod}</h4>
