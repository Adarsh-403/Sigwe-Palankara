...
export default function UserPanel() {
  ...
  const placeOrder = async () => {
    ...
    try {
      const payload = { items: cart.map(c => ({ productId: c._id, quantity: c.quantity, price: c.price, name: c.name })), totalAmount };
  ...
  return (
    <div className="min-h-screen bg-[#f5fbf5] relative overflow-x-hidden">
      ...
      {/* ── Floating decorations (only when products loaded) ── */}
      ...
      <div className="relative bg-gradient-to-r from-emerald-800 to-emerald-700 py-8 text-center overflow-hidden">
        ...
        <p className="relative text-emerald-200 text-sm mt-1">Natural products, honest prices</p>
      </div>
      ...
      <main className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-gray-500 font-medium text-sm">Fetching products…</p>
          </div>
        ) : products.length === 0 ? (
          /* Empty state with large cart illustration */
          <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
            ...
            <div>
              <p className="text-gray-600 font-semibold text-lg">No products yet</p>
              <p className="text-gray-400 text-sm mt-1">Check back soon — fresh items are on the way 🌿</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map(product => (
              <div key={product._id}
                className="group bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] overflow-hidden transition-all duration-300 hover:border-emerald-200 hover:shadow-[0_8px_24px_rgba(5,150,105,0.12)] hover:-translate-y-1.5">
                <div className="h-48 bg-[#f1f3f0] w-full overflow-hidden flex items-center justify-center relative">
                  {product.image ? (
                    ...
                    /* Placeholder with mini cart SVG */
                ...
                <div className="p-5">
                  ...
                  {product.stock > 0 && (
                    <p className="text-xs text-emerald-600 mb-3">{product.stock} in stock</p>
                  )}
                  <div className="flex items-center justify-between mt-auto">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 px-3 py-2 rounded-xl bg-emerald-50 text-emerald-700 hover:bg-emerald-600 hover:text-white transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed text-sm font-semibold"
                    >
                      <Plus size={16} /> Add
                    </button>
                  </div>
